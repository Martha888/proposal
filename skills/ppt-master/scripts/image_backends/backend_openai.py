...
def _is_chat_completions_image_model(model: str) -> bool:
    ...
    return "flash-image" in normalized or "image-preview" in normalized
...
def _chat_url_candidates(base_url: str) -> list[str]:
    normalized = base_url.rstrip("/")
    candidates = [_resolve_chat_url(normalized)]
    if normalized.endswith("/v1"):
        candidates.append(_resolve_chat_url(normalized[:-3]))
    return candidates


def _decode_data_url(data_url: str) -> tuple[bytes, str]:
    if not data_url.startswith("data:image/") or ";base64," not in data_url:
        raise ValueError("Expected a base64 data:image URL from chat completions.")
    header, encoded = data_url.split(",", 1)
    content_type = header[5:].split(";", 1)[0]
    return base64.b64decode(encoded), content_type


def _extract_chat_image_url(message) -> str | None:
    images = _field(message, "images")
    if images:
        first = images[0]
        image_url = _field(first, "image_url")
        if isinstance(image_url, str):
            return image_url
        if image_url:
            url = _field(image_url, "url")
            if url:
                return url
...
def _generate_chat_image(api_key: str, prompt: str,
                         aspect_ratio: str = "1:1", image_size: str = "1K",
                         output_dir: str = None, filename: str = None,
                         model: str = DEFAULT_MODEL, base_url: str = None) -> str:
    """Generate an image through OpenAI-compatible chat completions."""
    if not base_url:
        raise ValueError(
            "OPENAI_BASE_URL is required for chat-completions image models."
        )
    ...
    print(f"[OpenAI Chat - Proxy: {base_url}]")
    print(f"  Model:        {model}")
    ...
    response = None
    try:
        for url in _chat_url_candidates(base_url):
    ...
    if choices:
        image_url = _extract_chat_image_url(choices[0].get("message", {}))
        if image_url:
            path = resolve_output_path(prompt, output_dir, filename, ".png")
            if image_url.startswith("data:image/"):
                image_data, content_type = _decode_data_url(image_url)
                return save_image_bytes(image_data, path, content_type=content_type)
...
def _generate_image(api_key: str, prompt: str,
    ...
    """
    if _is_chat_completions_image_model(model):
        return _generate_chat_image(
            api_key,
            prompt,
            aspect_ratio,
            image_size,
